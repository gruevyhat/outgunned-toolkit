...
export function score(answers,data){
  ...
  const tropes=Object.entries(data.tropes.tropes).map(([id,t])=>({id,name:t.name,source:t.packName||'Corebook',score:t.attributes.reduce((n,a)=>n+(weights[`attr:${a}`]||0),0)+t.skills.reduce((n,s)=>n+(weights[`skill:${s}`]||0),0)+(universal?0:(weights[`trope:${id}`]||0)*3)})).sort(byScore)
  return {roles,tropes}
}
