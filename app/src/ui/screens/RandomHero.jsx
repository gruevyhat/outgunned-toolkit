...
export default function RandomHero({data,onBack,initial,onAddToCrew}){
  ...
  const edit=(field,value)=>{if(field==='personal.age'){try{setHero(generateRandom(makeRng(Date.now()),data,{role:hero.role,roleAttribute:hero.roleAttribute,trope:hero.trope,tropeAttribute:hero.tropeAttribute,age:value,name:hero.personal.name,job:hero.personal.job,catchphrase:hero.personal.catchphrase,flaw:hero.personal.flaw}))}catch{}return} const next=structuredClone(hero),parts=field.split('.');let parent=next;for(const p of parts.slice(0,-1))parent=parent[p];parent[parts.at(-1)]=value;if(!validateHero(next,data).length)setHero(next)}
