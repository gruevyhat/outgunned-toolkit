...
export default function RandomHero({data,onBack,initial,onAddToCrew,onImportMarkdown}){
  const [hero,setHero]=useState(()=>initial||generateRandom(makeRng(Date.now()),data)),[editing,setEditing]=useState(false)
  useEffect(()=>{if(initial)setHero(structuredClone(initial))},[initial])
  ...
  return <HeroSheet hero={hero} data={data} mode={editing?'edit':'play'} onHome={onBack} onReroll={reroll} onEdit={edit} onWorkingChange={setHero} onToggleEdit={()=>setEditing(x=>!x)} onNew={()=>setHero(generateRandom(makeRng(Date.now()),data))} onAddToCrew={()=>onAddToCrew?.(hero)} onImportMarkdown={onImportMarkdown}/>
}
