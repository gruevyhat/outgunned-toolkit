...
describe('hero PDF',()=>{
  ...
  it('renders the cited Johnny Reed fixture',async()=>{const hero=generateRandom(makeRng(19),gameData,johnny.pins);expect(hero.personal.name).toBe('Johnny Reed');expect(hero.feats).toEqual(['gunslinger','hard_to_kill','selfless']);const bytes=await drawHeroSheet(hero,gameData);expect((await PDFDocument.load(bytes)).getPageCount()).toBe(1)})
})
